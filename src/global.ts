...
import { getSdk, Sdk } from "@madnesslabs/thefloodteam-backend/dist/sdk";
  ...
  if (window && !(window as any).FloodTeam) {
    ...
    (window as any).FloodTeam = {
      init: async (
        ...
        window.addEventListener("floodteamUpload", async (event: any) => {
          ...
          try {
            ...
            document.body.dispatchEvent(
              new CustomEvent("floodteamSuccess", {
                detail: {
            ...
            document.body.dispatchEvent(
              new CustomEvent("floodteamError", {
                detail: {
                  event: event?.detail?.event,
                  target: event?.target,
                  error: err,
        ...
        window.addEventListener("floodteamSubmit", async (event: any) => {
          if (
            !event ||
            !event.detail ||
        ...
        window.addEventListener("floodteamFetch", async (event: any) => {
          if (
            !event ||
            !event.detail ||
            !event.detail.endpoint ||
