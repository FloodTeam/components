import { r as registerInstance, h, a as getElement } from './index-2bcfd230.js';
...
const GraphPaySplits = class {
    ...
    render() {
        return (h("div", { class: "graph-pay-splits-wrapper" }, this.graphTitle ? h("h1", null, this.graphTitle) : null, h("fireenjin-graph", { type: "doughnut", datasets: this.datasets, labels: this.labels }), this.users.map((record) => {
            var _a;
            return (h("ion-item", null, h("floodteam-avatar", { src: ((_a = record === null || record === void 0 ? void 0 : record.user) === null || _a === void 0 ? void 0 : _a.user_avatar)
                    ? record.user.user_avatar
                    : "./img/avatar.png", slot: "start" }), h("h4", null, record.user.user_name), record.payables && record.payables.length > 0 ? (record.payables.map((payable) => [
                h("p", null, payable.type === "percent"
                    ? payable.percent * 100 + "%"
                    : "Flat", " ", "- ", record.user.group.group_name),
                payable.payment ? (h("p", null, this.getPaymentText(payable.payment))) : null,
            ])) : (h("floodteam-error", { message: "No payment is tied to this payable!" })), h("ion-badge", { slot: "end" }, this.formatUSD(record.amount))));
        })));
    }
    get graphPaySplitsEl() { return getElement(this); }
    static get watchers() {
        return {
            "users": ["onUsersUpdate"]
        };
...
export { GraphPaySplits as fireenjin_graph_pay_splits };
