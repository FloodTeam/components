...
const FeedCard = class {
    constructor(hostRef) {
        ...
        this.floodteamFeedAction = index.createEvent(this, "floodteamFeedAction", 7);
    }
    render() {
        return (index.h("ion-card", { style: this.backgroundImage
                ? { backgroundImage: `url('${this.backgroundImage}')` }
                : null }, index.h("ion-item", { class: {
                "has-thumbnail": !!this.image
            } }, this.image && (index.h("ion-thumbnail", { slot: "start" }, index.h("img", { src: this.image }))), index.h("ion-label", { style: !this.image ? { paddingLeft: "0px" } : null }, index.h("h3", null, this.heading), this.subtext && index.h("h5", null, this.subtext), index.h("div", { class: "tag-list" }, this.tags && this.tags.map(tag => index.h("ion-chip", null, tag))), index.h("p", null, this.details))), index.h("ion-buttons", null, this.actionButtons &&
            this.actionButtons.map(actionButton => (index.h("ion-button", { size: actionButton.size ? actionButton.size : "large", fill: actionButton.fill ? actionButton.fill : "solid", color: actionButton.color ? actionButton.color : "success", href: actionButton.href ? actionButton.href : null, onClick: event => this.floodteamFeedAction.emit({
                    event,
                    name: actionButton.action
                }) }, actionButton.iconStart && (index.h("ion-icon", { name: actionButton.iconStart, slot: "start" })), actionButton.text, actionButton.iconEnd && (index.h("ion-icon", { name: actionButton.iconEnd, slot: "end" }))))))));
    }
};
FeedCard.style = feedCardCss;

exports.floodteam_feed_card = FeedCard;
