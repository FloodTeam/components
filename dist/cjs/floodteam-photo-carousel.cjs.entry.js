...
const PhotoCarousel = class {
    ...
    render() {
        var _a, _b;
        return (index.h(index.Host, null, index.h("floodteam-input-photo", { name: this.name, ref: (el) => (this.inputPhotoEl = el), path: `jobs/${this.jobId}/photos`, documentId: this.jobId, fileName: new Date().toISOString(), multiple: true }), ((_a = this.photos) === null || _a === void 0 ? void 0 : _a.length) ? (index.h("ion-badge", { color: this.badgeColor, class: "photo-pagination" }, this.currentSlide, " / ", this.photos.length)) : null, !this.hideAddButton ? (index.h("ion-fab", { class: "select-photo", onClick: (event) => this.selectFiles(event) }, index.h("ion-fab-button", { color: this.addButtonColor }, index.h("ion-icon", { name: "add" })))) : null, ((_b = this.photos) === null || _b === void 0 ? void 0 : _b.length) ? (index.h("ion-slides", { ref: (el) => (this.photoSlidesEl = el), pager: false, options: this.options }, this.photos.map((photo) => (index.h("ion-slide", null, index.h("ion-img", { src: photo.url })))))) : (index.h("floodteam-fallback", { icon: "image", message: "No Picture Taken..." }))));
    }
};
PhotoCarousel.style = photoCarouselCss;

exports.floodteam_photo_carousel = PhotoCarousel;
