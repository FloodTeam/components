...
const InputPhoto = class {
    constructor(hostRef) {
        ...
        this.fireenjinUpload = index.createEvent(this, "fireenjinUpload", 7);
    ...
    render() {
        return (index.h("div", null, index.h("div", { class: "upload-wrapper" }, index.h("div", { class: this.loading ? "photo is-loading" : "photo", style: {
                backgroundImage: this.photoUrl ? `url('${this.photoUrl}')` : null,
            }, onClick: (event) => this.triggerFileInput(event), onDrop: (event) => this.onDrop(event), onDragOver: (event) => this.onDrag(event), onDragEnter: () => this.onDragEnter(), onDragLeave: () => this.onDragLeave() }, !this.photoUrl && this.initials ? this.initials : null), this.showButton ? (index.h("ion-button", { fill: "clear", expand: "block", size: "small", onClick: (event) => this.triggerFileInput(event) }, this.buttonText, index.h("ion-icon", { name: "image", slot: "end" }))) : null), index.h("slot", null), index.h("input", { type: "file", onChange: (event) => this.selectFile(event), accept: "image/*", multiple: this.multiple })));
    }
    get photoUploaderEl() { return index.getElement(this); }
    static get watchers() {
        return {
            "value": ["onPhotoChange"]
        };
...
exports.floodteam_input_photo = InputPhoto;
