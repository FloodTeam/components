...
const InputFile = class {
    ...
    render() {
        return (index.h("ion-card", { class: { "drag-over": this.dragOver }, onDragEnter: (event) => this.onDragEnter(event), onDragOver: (event) => this.onDragOver(event), onDrag: (event) => this.onDrag(event), onDrop: (event) => this.onDrop(event), onDragLeave: (event) => this.onDragLeave(event), onClick: () => this.openFile() }, index.h("ion-item", { lines: "none" }, index.h("ion-icon", { name: this.icon ? this.icon : "document", slot: "start" }), index.h("div", null, index.h("h2", null, this.dragOver
            ? "Drop File Here"
            : this.label
                ? this.label
                : "Upload a File"), index.h("p", null, this.selectedFile
            ? this.selectedFile
            : this.defaultValue
                ? this.defaultValue
                : "Select a letterhead"))), index.h("input", { type: "file", onChange: (event) => this.selectFile(event), accept: this.accept ? this.accept : null, value: "blah" })));
    }
    get fileUploaderEl() { return index.getElement(this); }
    static get watchers() {
        return {
            "value": ["onFileChange"]
        };
...
exports.floodteam_input_file = InputFile;
